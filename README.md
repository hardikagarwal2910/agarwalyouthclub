# Agrawal Club Limited — Website

Static single-page site for **Agrawal Club Limited** (ACL), a private members' club
for the next generation of the Agrawal samaj in Ahmedabad.

- **Live:** https://agrawalyouthclub.com
- **Source design:** "The Library" direction (compiled from the design tool to a
  dependency-free, standalone `index.html` — no build step, no runtime).

## Files
- `index.html` — the entire website (inline CSS/JS, Google Fonts via CDN).
- `CNAME` — custom domain for GitHub Pages (`agrawalyouthclub.com`).
- `.nojekyll` — serve files as-is (skip Jekyll processing).

## Hosting
Served via GitHub Pages from the `main` branch, root (`/`).
